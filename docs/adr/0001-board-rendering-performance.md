...
4. **Re-bind the drag library after every commit, not just on navigation.** Hook the drag library's init into Livewire's `commit` hook (`succeed()` callback — fires once per request, after the DOM is patched), not only `livewire:navigated`, so it keeps working after any update. Deliberately not `morph.updated`: that fires once per morphed *element*, so hooking rebinding there re-runs destroy/recreate once per element touched instead of once per request. Verified against the installed Livewire 4.4.5 source (`js/hooks.js` / `dist/livewire.esm.js`) — this is what upstream boards get wrong and why dragging silently breaks after the first move.
